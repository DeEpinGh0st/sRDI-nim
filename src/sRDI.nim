import winim
import math
import argparse

var
  rdiShellcode32: seq[byte] = @[]
  rdiShellcode64: seq[byte] = @[]
proc littleEndianToUint32(bytes: seq[byte], startIndex: int): uint32 =
  result = 0u32
  for i in 0..3:
    result = result or (uint32(bytes[startIndex + i]) shl (i * 8))

proc littleEndianToUint16(bytes: seq[byte], startIndex: int): uint16 =
  result = uint16(bytes[startIndex]) or (uint16(bytes[startIndex + 1]) shl 8)

proc is64BitDLL(dllBytes: seq[byte]): bool =
  const
    machineIA64 = uint16(512)
    machineAMD64 = uint16(34404)
  let headerOffset = littleEndianToUint32(dllBytes,60)
  let machine = littleEndianToUint16(dllBytes, int(headerOffset) + 4)   
  if machine == machineIA64 or machine == machineAMD64:
    return true
  return false


proc pack(num: uint32): seq[byte] =
  result.add(byte((num shr 0) and 255))
  result.add(byte((num shr 8) and 255))
  result.add(byte((num shr 16) and 255))
  result.add(byte((num shr 24) and 255))


proc stringToBytes(s: string): seq[byte] =
  for i in s:
    result.add(byte(i))


proc getFileContents(path: string): seq[byte] = 
  var file: File
  file = open(path,FileMode.fmRead)
  var contents = readAll(file)
  file.close()
  stringToBytes(contents)

proc ror(val: uint32, rBits: uint32, maxBits: uint32): uint32 =
  let exp = uint32(2 ^ maxBits) - 1
  # 计算右移后的结果
  let rightShifted = (val and exp) shr (rBits mod maxBits)
  # 计算左移后的结果
  let leftShifted = (val shl (maxBits - (rBits mod maxBits))) and exp
  # 合并右移和左移的结果，得到循环右移的效果
  result = rightShifted or leftShifted


proc hashFunctionName(name: string): uint32 =
  var functionHash: uint32 = 0
  # 字符串转字节序列
  var function = stringToBytes(name) 
  function.add(0x00)
  for b in function:
    functionHash = ror(functionHash, 13, 32)
    functionHash += uint32(b)
  return functionHash

proc appendItemToSeq[T](src: seq[byte], dst: openArray[T]): seq[byte] = 
    result.add(src)
    for i in dst:
      result.add(cast[byte](i))

proc convertToShellcode(dllbytes, functionHash, userData: var seq[byte], flags: int): seq[byte] =
  if len(userData) == 0:
    userData = stringToBytes("None")
  var 
    bootstrap: seq[byte]
    bootstrapSize,dllOffset,userDataLocation: int
  if is64BitDLL(dllbytes):
    bootstrapSize = 64
    bootstrap = @[0xe8, 0x00, 0x00, 0x00, 0x00]
    dllOffset = bootstrapSize - bootstrap.len + rdiShellcode64.len
    bootstrap.add(0x59)
    bootstrap = appendItemToSeq(bootstrap,[0x49,0x89,0xc8])
    bootstrap = appendItemToSeq(bootstrap, [0x48, 0x81, 0xc1])
    # echo "dllOffset: " & $pack(uint32(dllOffset))
    bootstrap.add(pack(uint32(dllOffset)))
    bootstrap = appendItemToSeq(bootstrap, [0xba])
    bootstrap.add(functionHash)
    bootstrap = appendItemToSeq(bootstrap, [0x49, 0x81, 0xc0])
    userDataLocation = dllOffset + len(dllbytes)
    bootstrap.add(pack(uint32(userDataLocation)))
    #  mov r9d, <Length of User Data>
    bootstrap = appendItemToSeq(bootstrap, [0x41, 0xb9])
    bootstrap.add(pack(uint32(userData.len)))
    #  push rsi - save original value
    bootstrap = appendItemToSeq(bootstrap, [0x56])
    #  mov rsi, rsp - store our current stack pointer for later
    bootstrap = appendItemToSeq(bootstrap, [0x48, 0x89, 0xe6])
    #  and rsp, 0x0FFFFFFFFFFFFFFF0 - Align the stack to 16 bytes
    bootstrap = appendItemToSeq(bootstrap, [0x48, 0x83, 0xe4, 0xf0])
    #  sub rsp, 0x30 - Create some breathing room on the stack
    bootstrap = appendItemToSeq(bootstrap, [0x48, 0x83, 0xec])
    bootstrap = appendItemToSeq(bootstrap, [0x30]) #  32 bytes for shadow space + 8 bytes for last arg + 8 bytes for stack alignment
    #  mov dword ptr [rsp + 0x20], <Flags> - Push arg 5 just above shadow space
    bootstrap = appendItemToSeq(bootstrap, [0xC7, 0x44, 0x24])
    bootstrap = appendItemToSeq(bootstrap, [0x20])
    bootstrap.add(pack(uint32(flags)))
    #  call - Transfer execution to the RDI
    bootstrap = appendItemToSeq(bootstrap, [0xe8])
    #  Skip over the remainder of instructions
    bootstrap.add(byte(bootstrapSize-bootstrap.len-4))
    bootstrap = appendItemToSeq(bootstrap, [0x00, 0x00, 0x00])
    #  mov rsp, rsi - Reset our original stack pointer
    bootstrap = appendItemToSeq(bootstrap, [0x48, 0x89, 0xf4])
    #  pop rsi - Put things back where we left them
    bootstrap = appendItemToSeq(bootstrap, [0x5e])
    #  ret - return to caller
    bootstrap = appendItemToSeq(bootstrap, [0xc3])
    result.add(bootstrap)
    result.add(rdiShellcode64)
    result.add(dllbytes)
    result.add(userData)
  else:
    bootstrapSize = 45
    #  call next instruction (Pushes next instruction address to stack)
    bootstrap = @[0xe8, 0x00, 0x00, 0x00, 0x00]
    #  Set the offset to our DLL from pop result
    dllOffset = bootstrapSize - bootstrap.len + rdiShellcode32.len
    #  pop eax - Capture our current location in memory
    bootstrap = appendItemToSeq(bootstrap, [0x58])
    #  mov ebx, eax - copy our location in memory to ebx before we start modifying eax
    bootstrap = appendItemToSeq(bootstrap, [0x89, 0xc3])
    #  add eax, <Offset to the DLL>
    bootstrap = appendItemToSeq(bootstrap, [0x05])
    # echo "dllOffset: " & $pack(uint32(dllOffset))
    bootstrap.add(pack(uint32(dllOffset)))
    #  add ebx, <Offset to the DLL> + <Size of DLL>
    bootstrap = appendItemToSeq(bootstrap, [0x81, 0xc3])
    userDataLocation = dllOffset + dllBytes.len
    bootstrap.add(pack(uint32(userDataLocation)))
    #  push <Flags>
    bootstrap = appendItemToSeq(bootstrap, [0x68])
    bootstrap.add(pack(uint32(flags)))
    #  push <Length of User Data>
    bootstrap = appendItemToSeq(bootstrap, [0x68])
    bootstrap.add(pack(uint32(userData.len)))
    #  push ebx
    bootstrap = appendItemToSeq(bootstrap, [0x53])
    #  push <hash of function>
    bootstrap = appendItemToSeq(bootstrap, [0x68])
    bootstrap.add(functionHash)
    #  push eax
    bootstrap = appendItemToSeq(bootstrap, [0x50])
    #  call - Transfer execution to the RDI
    bootstrap = appendItemToSeq(bootstrap, [0xe8])
    #  Skip over the remainder of instructions
    bootstrap.add(byte(bootstrapSize-bootstrap.len-4))
    bootstrap = appendItemToSeq(bootstrap, [0x00, 0x00, 0x00])
    #  add esp, 0x14 - correct the stack pointer
    bootstrap = appendItemToSeq(bootstrap, [0x83, 0xc4, 0x14])
    #  ret - return to caller
    bootstrap = appendItemToSeq(bootstrap, [0xc3])
    result.add(bootstrap)
    result.add(rdiShellcode32)
    result.add(dllBytes)
    result.add(userData)

proc ShellcodeRDIFromBytes(data: var seq[byte], functionName, arguments: string): seq[byte] = 
  var
    clearHeader: bool = true
    userDataStr: string = arguments
    hashFunction,userData: seq[byte]
    hashFunctionUint32: uint32
    flags: int
  if functionName != "":
    hashFunctionUint32 = hashFunctionName(functionName)
    echo "[*] hashFunction: " & $hashFunctionUint32
    hashFunction = pack(hashFunctionUint32)
  else:
    hashFunction = pack(uint32(0x10))
  flags = 0
  if clearHeader:
    flags = flags or 0x1
  if userDataStr != "":
    userData = stringToBytes(userDataStr)
  result = convertToShellcode(data, hashFunction, userData, flags)

when isMainModule:
  const banner = """
     _____  _____ _____            _           
    |  __ \|  __ |_   _|          (_)          
 ___| |__) | |  | || |______ _ __  _ _ __ ___  
/ __|  _  /| |  | || |______| '_ \| | '_ ` _ \ 
\__ | | \ \| |__| _| |_     | | | | | | | | | |
|___|_|  \_|_____|_____|    |_| |_|_|_| |_| |_|
Author: S0cke3t
Github: https://github.com/DeEpinGh0st/sRDI-nim
                                                """
  var p = newParser:
    help(banner & "\nA nim implementation of sRDI")
    arg("dll", help="Specify the dll file to be converted.", nargs = 1)
    arg("function", help="Specify the dll export function to callback.", nargs = 1)
    arg("output", help="Specify the name of the shellcode file to output.", nargs = 1)
    option("-a", "--argments", help="Specifying Callback Function Parameters.")
  try:
    var
      dllContents, shellcode: seq[byte]
      funcName, exeArgs, dllPath, outputFile: string
      file: File
    var opts = p.parse()
    dllPath = opts.dll
    funcName = opts.function
    outputFile = opts.output
    exeArgs = opts.argments
    dllContents = getFileContents(dllPath)
    echo "[*] File size: " & $dllContents.len
    echo "[*] x64 bit dll: " & $is64BitDLL(dllContents)
    echo "[*] Convert dll path: " & dllPath
    echo "[*] Callback function name: " & funcName
    echo "[*] Callback function argments: " & exeArgs
    echo "[*] Output file name: " & outputFile
    shellcode = ShellcodeRDIFromBytes(dllContents, funcName, exeArgs)
    file = open(outputFile,FileMode.fmWrite)
    discard writeBytes(file, shellcode,0,shellcode.len - 1)
    echo "[√] Success: write " & $shellcode.len & " bytes to " & outputFile
  except ShortCircuit as err:
    if err.flag == "argparse_help":
      echo err.help
      quit(1)
  except UsageError:
    stderr.writeLine getCurrentExceptionMsg()
    quit(1)